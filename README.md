...
• Initialize a Go module:  VS Code → Terminal →  
`go mod init example.com/hello` *(Create this Module **go.mod** in C:\DEV\Repositories\GitHub\golang-server)*  
`go mod tidy` *(Adds any required dependencies and removes unused dependencies)*  
• Create folder: *C:\DEV\Repositories\GitHub\golang-server\static*  
• Add file: **index.html** with some HTML content.  
• Add a file: *C:\DEV\Repositories\GitHub\golang-server\api\hello.go*  
```
package handler
...
• Create *C:\DEV\Repositories\GitHub\golang-server\vercel.json*
```
{
...
• Create *C:\DEV\Repositories\GitHub\golang-server\local-main.go*
```
package main
...
• Run the program locally in VS Code:  
  `go run local-main.go`  *(Compiles and runs code)*  
